...
@NamedNativeQueries({
	...
	@NamedNativeQuery(name="Rate.Result", query="SELECT NUM ,      STR01 ,      STR02 ,      STR021 ,      STR03 ,      STR04 ,      STR05 ,      STR06 ,      STR07 ,      STR08 ,      STR09 ,      STR10 ,      STR11 ,      STR12 ,      STR13 ,      STR14 ,      STR15 ,      STR16 FROM      (SELECT Y.* ,             ROWNUM NUM        FROM             (SELECT X.RC_NO STR01 ,                    DECODE(X.MEET, '1', '서', '2', '제', '3', '부') STR02 ,                    DECODE(X.NORACE_FLAG, '1', '취소', NULL) STR021 ,                    DECODE(X.NORACE_FLAG, '1', '취소', NULL) STR03 ,                    DECODE(X.NORACE_FLAG, '1', NULL , NULL) STR04 ,                    DECODE(X.NORACE_FLAG, '1', NULL , NULL) STR05 ,                    DECODE(X.NORACE_FLAG, '1', NULL, NULL) STR06 ,                    DECODE(X.NORACE_FLAG, '1', NULL , NULL) STR07 ,                    DECODE(X.NORACE_FLAG, '1', NULL , NULL) STR08 , DECODE(X.NORACE_FLAG, '1', NULL, NULL) STR09 ,                    DECODE(X.NORACE_FLAG, '1', NULL , NULL) STR10,                    DECODE(X.NORACE_FLAG, '1', NULL, NULL) STR11,                    DECODE(X.NORACE_FLAG, '1', NULL, NULL) STR12 ,                    DECODE(X.NORACE_FLAG, '1', NULL , NULL) STR13 ,                    DECODE(X.NORACE_FLAG, '1', NULL , NULL) STR14 ,                    DECODE(X.NORACE_FLAG, '1', NULL , NULL) STR15 ,                    DECODE(X.NORACE_FLAG, '1', NULL , NULL) STR16               FROM                    (SELECT A.MEET ,                           A.RC_DATE ,                           A.RC_NO ,                           DECODE(SUM(DECODE(A.ORD,99,1,98,1,0)),0,0,1) AS NORACE_FLAG ,                           TO_CHAR(B.ST_TIME, 'HH24:MI') ST_TIME                      FROM RC_RESULT A ,                           RC_STAT B ,                           RC_SELL C                     WHERE A.MEET = B.MEET                           AND A.RC_DATE = B.RC_DATE                           AND A.RC_NO = B.RC_NO                           AND A.MEET = C.MEET                           AND A.RC_DATE = C.RC_DATE                           AND A.RC_NO = C.RC_NO                           AND A.RC_DATE = TO_DATE(?, 'YYYYMMDD')                           AND C.SEL_MEET = ?                           AND A.FINAL_BIT >= TO_NUMBER('1')  GROUP BY A.MEET ,                           A.RC_DATE ,                           A.RC_NO,                           TO_CHAR(B.ST_TIME, 'HH24:MI')) X           ORDER BY ST_TIME,                    MEET) Y)")
})
@Entity
